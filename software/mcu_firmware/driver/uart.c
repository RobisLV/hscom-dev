...
/***************************************************
*   eUSCI A0 UART function implementations
 **************************************************/
...
uint16_t UART_A1_parity(uart_setting uart_parity){
    if(uart_parity == UART_PARITY_ENABLE){
        MASK_SET(UCA1CTLW0, UCPEN);
    }
    else if(uart_parity == UART_PARITY_DISABLE){
        MASK_CLEAR(UCA1CTLW0, UCPEN);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_parity_mode(uart_setting uart_parity){
    if(uart_parity == UART_PARITY_ODD){
        MASK_SET(UCA1CTLW0, UCPAR);
    }
    else if(uart_parity == UART_PARITY_EVEN){
        MASK_CLEAR(UCA1CTLW0, UCPAR);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_stop_bits(uart_setting uart_stop_bits){
    if(uart_stop_bits==UART_STOP_2BIT){
        MASK_SET(UCA1CTLW0, UCSPB);
    }
    else if(uart_stop_bits==UART_STOP_1BIT){
        MASK_CLEAR(UCA1CTLW0, UCSPB);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_error_interrupt(uart_setting uart_error){
    if(uart_error == UART_ERROR_INT_ENABLE){
        MASK_SET(UCA1CTLW0, UCRXEIE);
    }
    else if(uart_error == UART_ERROR_INT_DISABLE){
        MASK_CLEAR(UCA1CTLW0, UCRXEIE);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_break_interrupt(uart_setting uart_break){
    if(uart_break == UART_BREAK_INT_ENABLE){
        MASK_SET(UCA1CTLW0, UCBRKIE);
    }
    else if(uart_break == UART_BREAK_INT_DISABLE){
        MASK_CLEAR(UCA1CTLW0, UCBRKIE);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_EUSCI_sleep(uart_setting uart_sleep){
    if(uart_sleep == UART_SLEEP_ENABLE){
        MASK_SET(UCA1CTLW0, UCDORM);
    }
    else if(uart_sleep == UART_SLEEP_DISABLE){
        MASK_CLEAR(UCA1CTLW0, UCDORM);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_transmit_address(uart_setting uart_adress){
    if(uart_adress == UART_TX_ADDRESS){
        MASK_SET(UCA1CTLW0, UCTXADDR);
    }
    else if(uart_adress == UART_TX_DATA){
        MASK_CLEAR(UCA1CTLW0, UCTXADDR);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_frame_break(uart_setting uart_break){
    if(uart_break == UART_TX_BREAK){
        MASK_SET(UCA1CTLW0, UCTXBRK);
    }
    else if(uart_break == UART_TX_NO_BREAK){
        MASK_CLEAR(UCA1CTLW0, UCTXBRK);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_deglitch_time(uart_setting uart_deglitch){
    MASK_CLEAR(UCA1CTLW1, UCGLIT0|UCGLIT1);
    if(uart_deglitch == UART_DEGLITCH_2NS){
        MASK_SET(UCA1CTLW1, UCGLIT_0);
    }
    else if(uart_deglitch == UART_DEGLITCH_50NS){
        MASK_SET(UCA1CTLW1, UCGLIT_1);
    }
    else if(uart_deglitch == UART_DEGLITCH_100NS){
        MASK_SET(UCA1CTLW1, UCGLIT_2);
    }
    else if(uart_deglitch == UART_DEGLITCH_200NS){
        MASK_SET(UCA1CTLW1, UCGLIT_3);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_clock_prescale(uint16_t uart_prescale){
    UCA1BRW = uart_prescale;
    return 0;
}

uint16_t UART_A1_modulation_stage_1(uint16_t uart_modulation){
    MASK_SET(UCA1MCTLW, uart_modulation&(0x00F0));
    return 0;
}

uint16_t UART_A1_modulation_stage_2(uint16_t uart_modulation){
    MASK_SET(UCA1MCTLW, uart_modulation&(0xFF00));
    return 0;
}

uint16_t UART_A1_oversampling(uart_setting uart_modulation){
    if(uart_modulation == UART_OVERSAMPLE_ENABLE){
        MASK_SET(UCA1MCTLW, UCOS16);
    }
    else if(uart_modulation == UART_OVERSAMPLE_DISABLE){
        MASK_CLEAR(UCA1MCTLW, UCOS16);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_RX_error_flag_read(void){
    if(MASK_CHECK(UCA1STATW, UCRXERR)){
        return 1;
    }
    return 0;
}
...
uint16_t UART_A1_TX_address_flag_read(void){
    if(MASK_CHECK(UCA1STATW, UCADDR)){
        return 1;
    }
    return 0;
}
...
uint16_t UART_A1_break_sync_length(uart_setting uart_delimiter_length){
    if(uart_delimiter_length == UART_DELIMITER_1BIT){
        MASK_CLEAR(UCA1ABCTL, UCDELIM0|UCDELIM1);
    }
    else if(uart_delimiter_length == UART_DELIMITER_2BIT){
        MASK_CLEAR(UCA1ABCTL, UCDELIM1);
        MASK_SET(UCA1ABCTL, UCDELIM0);
    }
    else if(uart_delimiter_length == UART_DELIMITER_3BIT){
        MASK_CLEAR(UCA1ABCTL, UCDELIM0);
        MASK_SET(UCA1ABCTL, UCDELIM1);
    }
    else if(uart_delimiter_length == UART_DELIMITER_4BIT){
        MASK_SET(UCA1ABCTL, UCDELIM0|UCDELIM1);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_sync_timeout(void){
    if(MASK_CHECK(UCA1ABCTL, UCSTOE)){
        return 1;
    }
    return 0;
}
...
uint16_t UART_A1_break_timeout(void){
    if(MASK_CHECK(UCA1ABCTL, UCBTOE)){
        return 1;
    }
    return 0;
}
...
uint16_t UART_A1_autobaud(uart_setting uart_autobaud){
    if(uart_autobaud == UART_BAUD_DET_ENABLE){
        MASK_SET(UCA1ABCTL, UCABDEN);
    }
    else if(uart_autobaud == UART_BAUD_DET_DISABLE){
        MASK_CLEAR(UCA1ABCTL, UCABDEN);
    }
    else{
        return 1;
    }
...
uint16_t UART_A1_TX_interrupt(uart_setting uart_tx_int){
    if(uart_tx_int == UART_INTERRUPT_ENABLE){
